'''
Runs TISSUE multiple imputation SpatialDE on specified dataset

Example: python get_multi_spatialde.py SCPI_k4_k1 Dataset11 10 none none none knn_spage_tangram --non-symmetric
'''
...
adata = group_multiple_imputation_testing_from_intermediate(dataset_name, methods, symmetric, condition, n_imputations=n_imputations,
                                                        group1=group1, group2=group2, savedir=savedir, test="spatialde")
                
# save results in anndata
preprocess_data(adata, standardize=False, normalize=False) # to keep consistent with predictions
adata.write(savedir+"/"+dataset_name+"_"+"_".join(methods)+"_MI_SPATIALDE.h5ad")
# if error loading (i.e. metadata too large), then large_save instead
try:
    adata2 = sc.read_h5ad(savedir+"/"+dataset_name+"_"+"_".join(methods)+"_MI_SPATIALDE.h5ad")
except:
    large_save(adata, savedir+"/"+dataset_name+"_"+"_".join(methods)+"_MI_SPATIALDE")
    os.remove(savedir+"/"+dataset_name+"_"+"_".join(methods)+"_MI_SPATIALDE.h5ad")